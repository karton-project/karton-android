 ...
 */
public class GameLevelBlockRecognitionProcessor extends VisionProcessorBase<FirebaseVisionText> {

    private static final String TAG = "TextRecProc";

    private final FirebaseVisionTextRecognizer detector;

    public GameLevelBlockRecognitionProcessor() {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if ((i < blocks.size() - 1)) {
                if (Math.abs(blocks.get(i).getCornerPoints()[0].y - blocks.get(i + 1).getCornerPoints()[0].y) < 20) {
                    Utils.gameLevelCode += blocks.get(i).getText() + " " +blocks.get(i + 1).getText();
                    i++;
                } else {
